...
int main(int count, char *buffer[]) {
  ...
  if (count >= 3) grep_launcher(&flag, count, buffer, pattern_buffer);
  return 0;
}
...
void grep_flags(int count, char *buffer[], flag *Flags, char *pattern_buffer) {
  ...
  while ((flag_value = getopt(count, buffer, f_options)) != -1) {
    switch (flag_value) {
      case 'e':
        Flags->flag_e = 1;
        snprintf(pattern_buffer, BUFFER_SIZE, "%s",
                 optarg);  // getopt(3) external variables
        break;
      case 'i':
        Flags->flag_i = 1;
        break;
...
void grep_file(flag *Flags, char *pattern, char *file_name) {
  ...
  if (Flags->flag_i) cflags = REG_ICASE;  // не учитывать регистр
  if (file != NULL) {
...
void grep_process(flag *Flags, FILE *file, regex_t reg, char *file_name) {
  ...
  while (fgets(text, BUFFER_SIZE - 1, file) !=
         NULL) {  // считывает данные с потока и делает из них строку
    bool match = 0;
    bool success =
        regexec(&reg, text, 1, pmatch, 0);  // успешность поиска слова
    if (strchr(text, '\n') == NULL)  // порядковый номер (с единицы)
      strcat(text, "\n");
    if (success == 0 && !Flags->flag_v) match = 1;
    if (success == REG_NOMATCH && Flags->flag_v) match = 1;
    ...
    if (match && Flags->flag_o) {
      regmatch_t pmatch[2];  // увеличиваем количество pmatch для обработки всех
                             // совпадений
      int offset = 0;
  ...
  if (Flags->flag_l && line_matches > 0) printf("%s\n", file_name);
  if (Flags->flag_c && !Flags->flag_l) printf("%d\n", line_matches);
}
